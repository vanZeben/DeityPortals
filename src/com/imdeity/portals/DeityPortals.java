...
public class DeityPortals extends JavaPlugin implements CommandExecutor {
    ...
    public void createTables() {
        Deity.data.getDB().Write(
                "CREATE TABLE IF NOT EXISTS " + Deity.data.getDB().tableName("deity_", "portals") + " (" + "`id` INT( 16 ) NOT NULL AUTO_INCREMENT PRIMARY KEY ,"
                        + "`command` VARCHAR( 128 ) NOT NULL ," + "`perform_from_console` INT( 1 ) NOT NULL ," + "`world` VARCHAR( 32 ) NOT NULL ," + "`min_x` INT( 16 ) NOT NULL ,"
                        + "`min_y` INT( 16 ) NOT NULL ," + "`min_z` INT( 16 ) NOT NULL ," + "`max_x` INT( 16 ) NOT NULL ," + "`max_y` INT( 16 ) NOT NULL ," + "`max_z` INT( 16 ) NOT NULL"
                        + ") ENGINE = MYISAM ;");
    }
        ...
        if ((query != null) && query.hasRows()) {
            for (int i = 0; i < query.rowCount(); i++) {
                try {
                    ...
                    Location minPoint = new Location(Deity.server.getServer().getWorld(query.getString(i, "world")), query.getInteger(i, "min_x"), query.getInteger(i, "min_y"), query.getInteger(i,
                            "min_z"));
                    Location maxPoint = new Location(Deity.server.getServer().getWorld(query.getString(i, "world")), query.getInteger(i, "max_x"), query.getInteger(i, "max_y"), query.getInteger(i,
                            "max_z"));
        ...
        if (sender instanceof Player) {
            ...
            if (args.length >= 2) {
                if (args[0].equalsIgnoreCase("add")) {
                    ...
                    sql = "INSERT INTO " + Deity.data.getDB().tableName("deity_", "portals")
                            + " (command, perform_from_console, world, min_x, min_y, min_z, max_x, max_y, max_z) VALUES (?,?,?,?,?,?,?,?,?);";
                    try {
                        Deity.data.getDB().Write(sql, commandToSend, Integer.parseInt(isConsole), minPoint.getWorld().getName(), (int) minPoint.getX(), (int) minPoint.getY(), (int) minPoint.getZ(),
                                (int) maxPoint.getX(), (int) maxPoint.getY(), (int) maxPoint.getZ());
                        ...
                        Deity.data.getDB().Write(sql, commandToSend, 0, minPoint.getWorld().getName(), (int) minPoint.getX(), (int) minPoint.getY(), (int) minPoint.getZ(), (int) maxPoint.getX(),
                                (int) maxPoint.getY(), (int) maxPoint.getZ());
                    ...
                    Deity.chat.sendPlayerMessageNoTitleNewLine(player, "&7===[&cDeityPortals&7]===");
                    try {
                        if (Deity.utils.paginateObject(tmp, 7, Integer.parseInt(args[1])) != null) {
                            for (String s : Deity.utils.paginateObject(tmp, 7, Integer.parseInt(args[1]))) {
                                Deity.chat.sendPlayerMessageNoTitleNewLine(player, s);
                            }
                        }
                    } catch (NumberFormatException e) {
                        if (Deity.utils.paginateObject(tmp, 7, 0) != null) {
                            for (String s : Deity.utils.paginateObject(tmp, 7, 0)) {
                                Deity.chat.sendPlayerMessageNoTitleNewLine(player, s);
                            }
                        }
                    }
